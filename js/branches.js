...
PathGen.findBranchPoints = function(nodemap) {
    ...
    for (var i = 0; i < nodemap.length; i++) {
        for (var j = 0; j < nodemap[i].length; j++) {
            if (nodemap[i][j].isVisited !== PathGen.MAIN_PATH) { continue; }

            var neighborcount = 0;
            if (j - 1 < 0 || nodemap[i][j-1].isVisited === PathGen.MAIN_PATH) { neighborcount++; }
            if (j + 1 >= nodemap[i].length || nodemap[i][j+1].isVisited === PathGen.MAIN_PATH) { neighborcount++; }
            if (i - 1 < 0 || nodemap[i-1][j].isVisited === PathGen.MAIN_PATH) { neighborcount++; }
            if (i + 1 >= nodemap.length || nodemap[i+1][j].isVisited === PathGen.MAIN_PATH) { neighborcount++; }
    ...
    for (var i = 0; i < branchpoints.length; i++) {
        ...
        for (var j = 0; j < branchLength; j++) {
            ...
            if (!backTrack) {
                ...
                if (randDir === 0)      { nodemap[prevx][prevy].n = branchId; nodemap[bx][by].s = branchId; }
                else if (randDir === 1) { nodemap[prevx][prevy].s = branchId; nodemap[bx][by].n = branchId; }
                else if (randDir === 2) { nodemap[prevx][prevy].w = branchId; nodemap[bx][by].e = branchId; }
                else                   { nodemap[prevx][prevy].e = branchId; nodemap[bx][by].w = branchId; }
            }
        }
    }
};
